...
class Continuation:
    ...
    def render(self, gui, start_coords, length=300):
        ...
        sx, sy = start_coords
        sx += 40
        wl, dl, bl = (int(length*p)
                for p in (self.white_f, self.draw_f, self.black_f)
        )
        gui.render_raw_text(self.forward, start_coords, gui.font_xs, (234, 234, 234))
        gfx.filled_polygon(gui.screen, 
    ...
    def __str__(self):
        return f"{self.forward}  |  {self.white}  {self.draw}  {self.black} | {self.total}"
